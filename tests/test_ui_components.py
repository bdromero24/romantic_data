...
def test_quote_cards_use_parchment_asset_background(tmp_path, monkeypatch) -> None:
    parchment_image = tmp_path / "perrgamino.png"
    parchment_image.write_bytes(b"fake-parchment")
    monkeypatch.setattr(components, "PARCHMENT_IMAGE", parchment_image)

    quotes_html = components.build_quote_cards_html(
        [
            {
                "message": "Un mensaje bonito",
                "sender": "Mar",
                "date": "10/06/2026",
            }
        ]
...
def test_first_te_amo_cards_render_heart_asset_only_there(
    tmp_path,
    monkeypatch,
) -> None:
    heart_image = tmp_path / "corazon.png"
    heart_image.write_bytes(b"fake-heart")
    monkeypatch.setattr(components, "HEART_IMAGE", heart_image)

    metric_html = components.build_metric_cards_html(
        [
            {
                "label": "Primer te amo",
                "value": "01/01/2026",
                "description": "Mar: te amo",
            },
            {
    ...
    timeline_html = components.build_timeline_html(
        [
            {
    ...
    assert metric_html.count("first-te-amo-heart") == 1
    assert timeline_html.count("first-te-amo-heart") == 1
    assert metric_html.count("first-te-amo-card") == 1
    assert timeline_html.count("first-te-amo-card") == 1
    assert "data:image/png;base64,ZmFrZS1oZWFydA==" in metric_html
    assert "data:image/png;base64,ZmFrZS1oZWFydA==" in timeline_html
...
def test_special_message_renders_configured_chat_blocks() -> None:
    ...
    assert "special-message-block-title" in special_html
    assert "Hola &lt;amor&gt;" in special_html
    assert "Yo &amp; tu" in special_html
    assert special_html.count('<section class="special-message-block ') == 2
...
def test_special_message_omits_empty_block_titles() -> None:
    special_html = components.build_special_message_html(
        {
            "blocks": [
                {
    ...
    assert "Sin titulo visible" in special_html
    assert "special-message-block-title" not in special_html
    assert "Cosas bonitas que ella me dijo" not in special_html
